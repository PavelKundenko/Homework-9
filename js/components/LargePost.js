...
export class LargePost extends Post {
  ...
  static MILLISECONDS_IN_HOUR = LargePost.MILLISECONDS_IN_DAY / 24;
  static MILLISECONDS_IN_MINUTE = LargePost.MILLISECONDS_IN_HOUR / 60;

  countPassedTimeFromDate = (date) => {
    const passedTime = new Date().getTime() - date.getTime();

    if (passedTime > LargePost.MILLISECONDS_IN_DAY) {
      return `${Math.floor(passedTime / LargePost.MILLISECONDS_IN_DAY)} days`;
    } else if (passedTime > LargePost.MILLISECONDS_IN_HOUR) {
      return `${Math.floor(passedTime / LargePost.MILLISECONDS_IN_HOUR)} hours`;
    } else {
      return `${Math.floor(passedTime / LargePost.MILLISECONDS_IN_MINUTE)} min`;
    }
  };

  renderComments = () => this._postData.comments
      .reduce((
          prevComments,
          {author, authorAvatar, text, rate, date},
          index,
          comments,
      ) => prevComments + `
    <li class="post__comment ${index + 1 === comments.length ? 'post__comment--last' : ''}">
      ...
      <div class="post__comment-info">
        ...
        <div class="post__comment-time">
          <img class="post__time-icon" src="img/a-icon-time.svg" alt="Clock icon">
          ${this.countPassedTimeFromDate(new Date(date))} ago
        </div>
      </div>
